...
namespace MagicLeap
{
    ...
    public class ControllerExample : MonoBehaviour
    {
        ...
        private void UpdateController()
        {
            ...
            {
                // MLMA - Touch Pad (not pressure sensitive).
                SetPressure(_padMask, (_controller.Touch1Active? 1 : 0));

                // MLMA - Bumper/Move
                _bumper.SetActive(_controller.State.ButtonState[(int)MLInputControllerButton.Move] != 0);

                // MLMA - Trigger/App
                _trigger.SetActive(_controller.State.ButtonState[(int)MLInputControllerButton.App] != 0);
                SetPressure(_triggerMask, 1);
            }
            ...
            if (_controller.Type == MLInputControllerType.Device)
            {
                if (_controller.Touch1Active)
                {
                    ...
                    if (_lastLEDindex != index)
                    {
                        ...
                        _controller.StartFeedbackPatternLED((MLInputControllerFeedbackPatternLED)index, MLInputControllerFeedbackColorLED.BrightCosmicPurple, 0);
                        _lastLEDindex = index;
                    }
                }
                else if (_lastLEDindex != -1)
                {
                    ...
                    // ideally, there'd be a way to turn off all the LEDs
                    _controller.StartFeedbackPatternLED((MLInputControllerFeedbackPatternLED)_lastLEDindex, MLInputControllerFeedbackColorLED.BrightCosmicPurple, 0.001f);
                    _lastLEDindex = -1;
                }
            }
        }
